...
| `BrowserType.launch: Executable doesn't exist at /opt/render/.cache/ms-playwright/...` | Render does not persist `~/.cache` between build and runtime containers | Set `PLAYWRIGHT_BROWSERS_PATH=0` in the Environment tab (and `render.yaml`) so browsers install into `.venv`, which is persisted |
